...
3. 案例檢索使用 `CASE_TOP_K = 6`。
4. 兩組結果合併後，會先依來源與文字內容指紋去除重複片段，降低不同手冊重複引用同一法條造成的雜訊。
5. 系統會將來源分類為正式法規、手冊 / 問答、裁罰案例與其他資料，並優先排序正式法規。
6. 食品添加物相關問題會優先檢索與排序「食品添加物使用範圍及限量暨規格標準」等正式規範。
7. 最多保留 `MAX_CONTEXTS = 12` 筆參考資料。
8. 每筆參考資料會標註來源分類、檢索類型、來源、相似度與文字內容。
...
2. 每段參考資料都會包含檢索類型、來源與相似度。
3. 系統 prompt 會要求 Gemini 扮演「台灣食品安全法規專家助理」。
4. Prompt 明確限制回答規則：
   - 必須使用繁體中文。
   - 必須依固定三點格式回答。
   - 第 1 點要優先根據正式法規說明法規依據。
   - 第 2 點要整理判斷與風險；只有在資料真的提供案例時才引用裁罰案例。
   - 第 3 點要提供合規建議；若使用者詢問標籤、廣告或文案，才提供可直接替換的修改建議。
   - 不可捏造法條、年份、金額或案例。
   - 資料不足時要明確說明。
   - 手冊、問答或指引只能作為輔助說明，不應取代正式法規。
5. 生成模型使用 `gemini-3.5-flash`。
6. 最後由 `model.generate_content(prompt)` 產生回答文字。

目的：

透過明確的 prompt 約束回答格式與資料來源，降低大型語言模型幻覺，並讓回答更符合食品法規問答的使用情境。
...
1. 加入 reranking 模型，提高檢索段落的精準度。
2. 將回答格式依問題類型動態調整，例如法條查詢、違規廣告、檢驗方法分流。
3. 建立管理介面，讓使用者可更新法規資料並重新建立索引。
4. 增加評測資料集，定期測試回答正確率與引用品質。
